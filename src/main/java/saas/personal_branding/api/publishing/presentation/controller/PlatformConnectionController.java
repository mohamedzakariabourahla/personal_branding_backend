...
import saas.personal_branding.api.publishing.presentation.dto.request.PlatformOAuthCallbackRequest;
import saas.personal_branding.api.publishing.presentation.dto.response.OAuthCompletionResult;
import saas.personal_branding.api.publishing.presentation.dto.response.PlatformAuthorizationResponse;
import saas.personal_branding.api.publishing.presentation.dto.response.PlatformConnectionResponse;
import saas.personal_branding.api.publishing.presentation.dto.response.PlatformSelectionCandidateResponse;
import saas.personal_branding.api.publishing.presentation.mapper.PlatformConnectionDtoMapper;
...
public class PlatformConnectionController {
    ...
    public ResponseEntity<OAuthCompletionResult> completeMetaOAuth(@Valid @RequestBody PlatformOAuthCallbackRequest request) {
        Long userId = authenticatedUserProvider.getCurrentUserId();
        try {
            var connection = metaOAuthApplicationService.completeAuthorization(userId, request.getState(), request.getCode(), request.getPageId());
    ...
    public ResponseEntity<OAuthCompletionResult> completeTikTokOAuth(@Valid @RequestBody PlatformOAuthCallbackRequest request) {
        Long userId = authenticatedUserProvider.getCurrentUserId();
        try {
            var connection = tikTokOAuthApplicationService.completeAuthorization(userId, request.getState(), request.getCode());
            return ResponseEntity.ok(OAuthCompletionResult.connected(PlatformConnectionDtoMapper.toResponse(connection)));
        } catch (PlatformException.InvalidAccountSelectionException | PlatformException.NoEligibleAccountException ex) {
            return ResponseEntity.ok(OAuthCompletionResult.failure(ex.getMessage()));
        } catch (PlatformException.ProviderCommunicationException ex) {
    ...
    public ResponseEntity<OAuthCompletionResult> completeYouTubeOAuth(@Valid @RequestBody PlatformOAuthCallbackRequest request) {
        Long userId = authenticatedUserProvider.getCurrentUserId();
        try {
            var connection = youtubeOAuthApplicationService.completeAuthorization(userId, request.getState(), request.getCode(), request.getPageId());
            return ResponseEntity.ok(OAuthCompletionResult.connected(PlatformConnectionDtoMapper.toResponse(connection)));
        } catch (PlatformException.SelectionRequiredException ex) {
            return ResponseEntity.ok(OAuthCompletionResult.selectionRequired(mapCandidates(ex.getCandidates())));
        } catch (PlatformException.InvalidAccountSelectionException | PlatformException.NoEligibleAccountException ex) {
